...
## 4. 대기열 승인 플로우 (`app/mobile/hooks/useQueue.ts`)

### 4.1 슬롯 배정

| 항목 | 내용 |
|------|------|
| **목적** | 큐 위치 기준으로 게임 슬롯 배정 |
| **입력** | `status-queue` 이벤트의 socket id 배열 |
| **출력** | `1 \| 2 \| 3 \| 4 \| null` |
| **동작** | 큐 위치 0~3을 슬롯 1~4로 매핑 |
...
| QF-1-2 | 네번째 플레이어 | position `3` | `4` |
| QF-1-3 | 다섯번째 플레이어 | position `4` | `null` |
...
| 항목 | 내용 |
|------|------|
| **목적** | 4명 미만 입장 시 대기 후 게임 시작 |
| **입력** | 현재 큐 인원 |
| **출력** | 게임 입장 또는 대기 유지 |
| **동작** | 4명 도달 시 즉시 시작, 1~3명은 30초 후 자동 시작 |

#### 테스트 케이스

| ID | 케이스 | 상황 | 예상 결과 |
|----|--------|------|----------|
| QF-2-1 | 4명 도달 | 큐 인원 4명 | 즉시 게임 입장 |
| QF-2-2 | 4명 미만 | 큐 인원 1~3명 | 30초 후 게임 입장 |
| QF-2-3 | 대기열 초과 | position 4 이상 | 대기 화면 유지 |
