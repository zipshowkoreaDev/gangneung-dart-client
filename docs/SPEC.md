...
|------|------|
| **목적** | 플레이어 방 이름 생성 |
| **입력** | `baseRoom: string`, `playerSlot: 1 \| 2 \| 3 \| 4` |
...
| RM-1-3 | 슬롯 4 | `("zipshow", 4)` | `"game-zipshow-player4"` |
...
### 2.3 getSlotFromPosition

| 항목 | 내용 |
|------|------|
| **목적** | room 내 참가자 위치를 플레이어 슬롯으로 변환 |
| **입력** | `position: number` |
| **출력** | `1 \| 2 \| 3 \| 4 \| null` |
| **조건** | position 0~3만 슬롯으로 변환 |
...
| RM-3-1 | 첫 번째 참가자 | `0` | `1` |
| RM-3-2 | 네 번째 참가자 | `3` | `4` |
| RM-3-3 | 다섯 번째 참가자 | `4` | `null` |
| RM-3-4 | 음수 위치 | `-1` | `null` |

---

### 2.4 extractPlayerSlot
...
| RM-5-3 | 플레이어4 | `"game-zipshow-player4"` | `4` |
| RM-5-4 | 디스플레이 | `"game-zipshow-display"` | `null` |
| RM-5-5 | 잘못된 형식 | `"invalid-room"` | `null` |
...
| 항목 | 내용 |
|------|------|
| **목적** | room 참가자 위치 기준으로 게임 슬롯 배정 |
| **입력** | `joinedRoom`의 socket id 배열 |
| **출력** | `1 \| 2 \| 3 \| 4 \| null` |
| **동작** | 참가자 위치 0~3을 슬롯 1~4로 매핑 |

#### 테스트 케이스

| ID | 케이스 | 입력 | 예상 출력 |
|----|--------|------|----------|
| QF-1-1 | 첫 플레이어 | position `0` | `1` |
| QF-1-2 | 네 번째 플레이어 | position `3` | `4` |
| QF-1-3 | 다섯 번째 플레이어 | position `4` | `null` |
...
| 항목 | 내용 |
|------|------|
| **목적** | 로비 인원 기준 게임 시작 가능 여부 판단 |
| **입력** | 현재 room 인원 |
| **출력** | 게임 입장 또는 대기 유지 |
| **동작** | 상위 4명만 이번 세션 참가 후보가 되며 1번 플레이어가 시작을 승인 |

#### 테스트 케이스

| ID | 케이스 | 상황 | 예상 결과 |
|----|--------|------|----------|
| QF-2-1 | 4명 도달 | room 인원 4명 | 시작 가능 |
| QF-2-2 | 4명 미만 | room 인원 1~3명 | host 승인 대기 |
| QF-2-3 | 정원 초과 | room 인원 5명 이상 시도 | `roomFull`로 진입 차단 |
...
| `disconnect-room` | `{ room }` | room 세션 종료 |
